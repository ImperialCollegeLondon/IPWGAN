...
def main():
    ...
    work_dir = os.path.expandvars('benpmforfine/')
    ...
    n_levels = 4  
    netG = WGANFPN.LaplacianPyramidGenerator(opt.imageSize, nz, nc, ngf, ngpu).to(device)
    netG.apply(weights_init)
    if opt.netG != '':
        netG.load_state_dict(torch.load(opt.netG))
    print(netG)

    netD = WGANFPN.WGAN3D_D(opt.imageSize, nz, nc, ndf, ngpu).to(device)
    netD.apply(weights_init)
    if opt.netD != '':
        netD.load_state_dict(torch.load(opt.netD))
    print(netD)

    wasserstein_criterion = WGANFPN.WassersteinLP(opt.weight_clip_value, weight_gradient_penalty_weight, netD)
